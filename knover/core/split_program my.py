...
def op_inputs(op, src_block, dst_block):
    ...
    for i in range(0, len(op.input_names)):
        ...
        for var_name in var_names:
            if not src_block.has_var(var_name):
                continue
...
def find_var(block):
    ...
    for op in ops:
        for i in range(0, len(op.input_names)):
            ...
            var_names = op.input(param_name)
            for var_name in var_names:
                if block.has_var(var_name) and not var_name.startswith("_generated_var"):
                    if block.var(var_name).shape == (-1, 16, 1, 2):
                        print(var_name, block.var(var_name).shape)
...
def replace(src_program, dst_program, \
    ...
    for i in range(src_block_start_op_idx, src_block_start_op_idx+op_count-5):
        ...
        if i == 44:
            import pdb
            pdb.set_trace()
        if dst_block.var("concat_49.tmp_0").shape == (-1, 2, 1024): # 44
            import pdb
            pdb.set_trace()
        # if dst_block.var("concat_49.tmp_0").shape == (-1, 2, 16, 64): # 50
        #     import pdb
        #     pdb.set_trace()
        # if dst_block.var("transpose_97.tmp_0").shape == (-1, 16, 2, 64): # 51epoch
        #     import pdb
        #     pdb.set_trace()
        # if dst_block.has_var("matmul_48.tmp_0"):
        #     import pdb
        #     pdb.set_trace()
        # if i == 56:
        #     import pdb
